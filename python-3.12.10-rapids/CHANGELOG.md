...
- 9365306: Fix the RAPIDS runenv `PATH` env var breaking every non-docker run: use `$PATH` instead of `${PATH}`.
...
- dbdc64c: Add CuPy and NVIDIA CUDA libraries to RAPIDS runenv

  - cupy-cuda12x>=13.0.0
  - nvidia-cuda-nvrtc-cu12 (NVRTC for CuPy JIT compilation)
